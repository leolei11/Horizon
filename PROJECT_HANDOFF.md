...
- 中文正文补全成功 20/20，无降级。
- 使用正式 `data/pushed_history.json` 再模拟明日运行：抓取 69 条、跨源合并 68 条、历史去重后 55 条；质量优先线以上 28 条，最终审计后有 34 个唯一候选，精确选出 20 条。
...
代码验证：507 项 Python 测试全部通过，Remotion TypeScript 类型检查、配置加载、工作流 YAML 解析与 `git diff --check` 均通过。
